...
mod tests {
    ...
    fn merge_sort_test() {
        ...
        merge_sort(&mut data, 0, length - 1);
        merge_sort(&mut data1, 0, length1 - 1);
        assert_eq!(data, [2, 3, 5, 6, 9]);
        assert_eq!(data1, [1, 2, 3, 4, 5]);
    }
}
...
pub fn merge_sort(unsorted_data: &mut [i32], p: usize, r: usize) {
    if p < r {
        let q = (p + r) / 2;
...
pub fn merge(unsorted_data: &mut [i32], p: usize, q: usize, r: usize) -> () {
